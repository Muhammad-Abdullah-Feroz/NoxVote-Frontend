...
const ElectionPage = ({ userEmail }) => {
  ...
  useEffect(() => {
    const fetchElections = async () => {
      try {
        const res = await fetch(`http://localhost:5000/get_user_polls?user=${userEmail}`);
        if (!res.ok) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const fetchElections = async () => {
          try {
            const resE = await fetch(`http://localhost:5000/get_user_polls?user=${userEmail}`);
            if (!resE.ok) {
  ...
  return (
    <div className="bg-gray-900 text-white min-h-[90%] p-8">
          ...
          >
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                type="submit"
                className="bg-blue-600 w-full py-2 mt-4 cursor-pointer rounded font-semibold hover:bg-blue-700 transition"
              >
                Submit Election
              </motion.button>
            </form>
      ...
      <PollStat poll={selectedElection} onClose={() => setSelectedElection(null)} />
