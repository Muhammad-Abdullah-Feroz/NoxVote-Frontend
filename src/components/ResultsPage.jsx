...
const ResultsPage = () => {
    ...
    return (
        <div className="bg-gray-900 text-white min-h-screen p-8">
            ...
            {loading ? (
                <p className="text-center text-gray-400 text-lg">Loading results...</p>
            ) : pastPolls.length === 0 ? (
                <p className="text-center text-gray-400 text-lg">No past elections available.</p>
            ) : (
                <motion.div layout className="space-y-6 max-w-3xl mx-auto">
                    {pastPolls.map((poll) => {
                        ...
                        return (
                            ...
                            >
                                <h2 className="text-2xl font-semibold mb-1">{poll.poll_name}</h2>
                                <p className="text-gray-400 mb-4 text-sm">Ended on: {poll.closing_date}</p>
                                ...
                                {Object.keys(votes).length === 0 ? (
                                    ...
                                    <div className="space-y-2">
                                        {Object.entries(votes).map(([candidate, count]) => (
                                            <div key={candidate} className="bg-gray-700 px-4 py-2 rounded">
                                                <p className="flex justify-between">
                                                    <span className="font-medium">{candidate}</span>
                                                    <span>{count} votes ({calculatePercent(count, totalVotes)}%)</span>
                                                </p>
                                                <div className="w-full bg-gray-600 rounded-full h-2 mt-1">
                                                    <div
                                                        className="bg-green-500 h-2 rounded-full"
                                                        style={{ width: `${calculatePercent(count, totalVotes)}%` }}
                                                    />
                                                </div>
                                            </div>
                                        ))}
